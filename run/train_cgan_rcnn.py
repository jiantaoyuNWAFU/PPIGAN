...
from Discriminator import Dis, Dis_rcnn,Dis_cnn
from Generator import Gen
from CGAN import cgan
from dataset import MyDataset
from helper import convert_to_acid_ids_pytorch
...
def train(args):
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            if accuracy > best_acc:
                ...
                if args.is_only_dis:
                    torch.save(D.state_dict(),
                               os.path.join('./models/only_dis/human/',
                                            f'D_best_{args.epoch}_gen(x,z)_randomselect_cnn.pth'))
                else:
                    torch.save(D.state_dict(),
                               os.path.join('./models/dis_after_gen/human/',
                                            f'D_best_{args.beta_real_loss}_gen(x,z)_randomselect_cnn_200.pth'))
                    torch.save(G.state_dict(),
                               os.path.join('./models/dis_after_gen/human/',
                                            f'G_best_{args.beta_fake_loss}_gen(x,z)_randomselect_cnn_200.pth'))
            ...
            print("===============================================")
            if args.is_only_dis:
                with open(f"./log/human/log_only_dis.txt", "a+") as f:
                    f.write(
                    ...
                    f.write("===============================================\n")
                continue
            with open(f"./log/human/log_{args.beta_real_loss}_{args.beta_fake_loss}_position.txt", "a+") as f:
                f.write(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interaction_data',
                        default="/home/hlw/gengjing/seq_ppi-master/sun/preprocessed/Supp-AB.tsv", type=str)
    parser.add_argument('--sequence_data',
                        default="/home/hlw/gengjing/seq_ppi-master/sun/preprocessed/SEQ-Supp-ABCD.tsv", type=str)
    parser.add_argument('--epoch', type=int, default=15, help='the maximum number of epochs')
    parser.add_argument('--train_dataset', default="", required=False, type=str, help="The path of the train dataset")
    parser.add_argument('--test_dataset', default="", required=False, type=str, help="The path of the test dataset")
    # parser.add_argument('--emb_file', default="D:\\gengjing\\exp\\deeptrio-master\\deeptrio-pytorch\\embeddings\\vec5_CTC.txt", required=False, type=str, help="The path of the embedding file")
    parser.add_argument('--d_pth', default="", required=False, type=str, help="The path of the discriminator model")
    ...
    parser.add_argument('--batch_size', default=16, type=int, help='The batch size of the training process')
    ...
    parser.add_argument('--hidden_dim', default=25, type=int, help='The dimension of the embedding layer')
    ...
    args.d_pth ="/home/hlw/gengjing/deeptrio-pytorch1/models/only_dis/human/D_best_50_gen(x,z)_randomselect_cnn.pth"
    args.epoch = 200
