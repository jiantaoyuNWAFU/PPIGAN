...
class Gen(nn.Module):
    ...
    def forward(self, input_a, input_b):
        ...
        #embedded_a = F.one_hot(input_a.long(), num_classes=21).float()
        #input_b = input_b.long()
        #print(input_b.shape)
...
class Generator(nn.Module):
    ...
    def forward(self, input_a, input_b):
        ...
        x = self.fc(x)
        x = x.unsqueeze(2)  # Add a dimension for ConvTranspose1d
...
if __name__ == "__main__":
    ...
    print(test_res)
    # for name, layer in G.named_modules():
    #     print(f'{name}: {layer}')